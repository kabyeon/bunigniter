...
export class MemoryQueueDriver implements QueueDriver {
	...
	pop(queueName: string): JobPayload | null {
		...
		const index = queue.findIndex(j =>
			j.availableAt <= now && j.reservedAt === null,
		);
	...
	size(queueName: string): number {
		const now = Date.now();
		return this.getQueue(queueName).filter(j =>
			j.reservedAt === null && j.availableAt <= now,
		).length;
	}
...
export class RedisQueueDriver implements QueueDriver {
	...
	async pop(queueName: string): Promise<JobPayload | null> {
		...
		const results = await this.client.send("ZRANGEBYSCORE", [key, "0", now.toString(), "LIMIT", "0", "1"]);
