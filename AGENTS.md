...
| `core/router.ts` | 라우트 정의 → Bun.serve routes 변환(`toBunServe()`), `resource()`/`group()`, 미들웨어 파이프라인, 라우트 모델 바인딩 | controller.ts, middleware.ts, route_model_binding.ts |
...
- 현재: **598 pass, 0 fail, 1071 expect() calls across 27 files**
